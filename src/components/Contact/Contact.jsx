...
const Contact = () => {
  const [formData, setFormData] = useState({
    ...
    goal: '', business_type: '', other_business: '', message: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ full_name: '', email: '', phone: '', website_url: '', goal: '', business_type: '', other_business: '', message: '' });
      setTouched({});
    } catch {
          ...
          <Row className="justify-content-center g-4">
            <Col xs={12} md={5}>
              <div className="d-flex align-items-center p-4 rounded shadow-sm bg-white">
                ...
                <div><p className="mb-0">+880 1947349917</p></div>
              </div>
            </Col>
            <Col xs={12} md={5}>
              <div className="d-flex align-items-center p-4 rounded shadow-sm bg-white">
                <FaEnvelope className="text-warning fs-3 me-3" />
                <div><p className="mb-0">khdreamit@gmail.com</p></div>
              </div>
            </Col>
          </Row>
          <Row className="justify-content-center g-3 mt-5">
            <Col xs="auto">
              <Button href="https://www.facebook.com/AdulHalim.net/" target="_blank"
        ...
        <Container>
          ...
          <Row className="align-items-center g-5">
            <Col xs={12} md={5} className="text-center">
              <img src={leadImg} alt="contact" className="img-fluid rounded shadow"
                  ...
                  <Row className="g-3">
                    <Col xs={12} md={6}>
                      ...
                      <input type="text" name="full_name" value={formData.full_name}
                        ...
                        className={getFieldClass('full_name')} placeholder="Your full name" />
                      {touched.full_name && errors.full_name && <div className="invalid-feedback">{errors.full_name}</div>}
                    </Col>
                    <Col xs={12} md={6}>
                      <label className="form-label fw-semibold">Email Address <span className="text-danger">*</span></label>
                      <input type="text" name="email" value={formData.email}
                        onChange={handleChange} onBlur={handleBlur}
                        className={getFieldClass('email')} placeholder="your@email.com" />
                    ...
                    <Col xs={12} md={6}>
                      ...
                      <input type="text" name="phone" value={formData.phone}
                        onChange={handleChange} onBlur={handleBlur}
                        className={getFieldClass('phone')} placeholder="+880 1XXXXXXXXX" />
                    ...
                    <Col xs={12} md={6}>
                      ...
                      <input type="text" name="website_url" value={formData.website_url}
                        onChange={handleChange} className="form-control" placeholder="https://yourwebsite.com" />
                    </Col>
                    <Col xs={12}>
                      <label className="form-label fw-semibold">What is your goal? <span className="text-danger">*</span></label>
                      <select name="goal" value={formData.goal} onChange={handleChange} onBlur={handleBlur}
                      ...
                      {touched.goal && errors.goal && <div className="invalid-feedback">{errors.goal}</div>}
                    </Col>
                    <Col xs={12}>
                      ...
                      <select name="business_type" value={formData.business_type}
                        onChange={handleChange} onBlur={handleBlur} className={getSelectClass('business_type')}>
                      ...
                      {touched.business_type && errors.business_type && <div className="invalid-feedback">{errors.business_type}</div>}
                    </Col>
                    {formData.business_type === 'Other' && (
                      <Col xs={12}>
                        <label className="form-label fw-semibold">Please specify <span className="text-danger">*</span></label>
                        <input type="text" name="other_business" value={formData.other_business}
                          onChange={handleChange} onBlur={handleBlur}
                          className={getFieldClass('other_business')} placeholder="Your business type" />
                        {touched.other_business && errors.other_business && <div className="invalid-feedback">{errors.other_business}</div>}
                      </Col>
                    )}
                    <Col xs={12}>
                      <label className="form-label fw-semibold">Message</label>
                      <textarea name="message" value={formData.message} onChange={handleChange}
                        className="form-control" rows={4} placeholder="Write your message here..." />
                    </Col>
                    <Col xs={12} className="text-center mt-2">
                      <button type="submit" disabled={!isFormValid || loading}
                        className="btn btn-primary px-5 py-2 fw-bold"
                        style={{ fontSize: '16px', borderRadius: '8px', opacity: isFormValid ? 1 : 0.4, cursor: isFormValid ? 'pointer' : 'not-allowed', transition: 'opacity 0.3s' }}>
